...
class ExtJSHelper(Page):
    ...
    def _click_element_js(self, selector_section):
        ...
        logger.debug('javascript to trigger click: {0}'.format(my_query))
        self.execute_javascript(my_query)
        return self
        
    def _click_element_from_js_clsname(self, selector_section, position=0):
        """ Click an element in the html page by executing javascript
        Use the method seldom on running out of option
        Args:
            selector_section is 'css'
        Returns:
            None
        """

        my_query = 'document.getElementsByClassName("{0}")[{1}].click()'.format(selector_section, position)
        logger.debug('javascript to trigger click: {0}'.format(my_query))
        self.execute_javascript(my_query)
        return self        
    ...
    def _get_active_tab(self):
        ...
        active_tab_id = self._get_gridid(self.custom_selectors['ActiveTab'])
    ...
    def close_tab(self, active_tab_id):
        ...
        Args:
           active_tab_id: id of (active tab in centertabpanel)
        """

        self.click_element(self._append_closeEl(self._get_active_tab()))
        return self
...
class SystemMenu(ExtJSHelper):
    ...
    def click_users(self):
        ...
        #self._click_element_from_js_clsname("x-tree-elbow-img x-tree-elbow-plus x-tree-expander", 0)
        #self.click_element("css=.x-tree-elbow-img.x-tree-elbow-plus.x-tree-expander")
        self.click_element("css=#treeview-1023-record-11 .x-grid-cell-inner .x-tree-elbow-img")
        return self
    ...
    def click_connections(self):
        self._get_id_for_systemcomponents()
        #self._click_element_from_js_clsname("x-tree-elbow-img x-tree-elbow-plus x-tree-expander", 1)
        self.click_element("css=#treeview-1023-record-12 .x-grid-cell-inner .x-tree-elbow-img")
        #self.click_element("css=#treeview-1023-record-11 .x-tree-elbow-img + img")
...
class ProcessTab(ExtJSHelper):
    ...
    def toggle_column(self, column_name='Process Name'):
        ...
        self._click_element_js(id_triggerEl)          # looks as execute_javascript('return document.getElementById("gridcolumn-
        ...
        logger.debug('menucheck item id is  {0}'.format(id_of_menucheckitem))         #looks as Ext.ComponentQuery.query("menu menuitem [text = Process Name]")[0].id
        self.click_element(id_of_menucheckitem)
        return self

    def type_input_process_name(self):
        self.input_text("Process Name", "CLEAN%")
        return self

    def type_input_process_description(self):
        self.input_text("Process Desc Input", "Clean%")
        return self
    ...
    def click_retreive_button(self):
        xpath_for_button = "processPanel reportTopBar button"
        logger.debug(xpath_for_button)
        id_element = self._get_gridid(xpath_for_button)
